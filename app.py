...
st.markdown("""
<style>
:root { --primary-blue:#3B82F6; --primary-cyan:#06B6D4; --error-red:#EF4444; }
...
.card { padding: 1rem; border: 1px solid #E5E7EB; border-radius: 8px; background:#fff; }
...
def compute_bmi(height_cm: float, weight_kg: float):
    """Return (bmi_value, category, adjustment)."""
    if height_cm <= 0 or weight_kg <= 0:
        return 0.0, "Invalid", 0
    h_m = height_cm / 100.0
    bmi = weight_kg / (h_m ** 2)
    if bmi < 18.5:   return bmi, "Underweight", -200
    elif bmi <= 25:  return bmi, "Normal", 0
    else:            return bmi, "Overweight", 200
...
def condition_adjustment(condition: str) -> int:
    return {
        "None": 0, "Athlete": 300, "Hot climate": 200,
        "Pregnant": 250, "Diabetic": 150, "Kidney/Heart": -300,
    }.get(condition, 0)
...
def draw_bottle(progress_pct: float, width=220, height=420):
    """Draws simple rectangles (no rounded_rectangle) for maximal Pillow compatibility."""
    ...
    # Neck (rectangle)
    neck_left, neck_right = width//2 - 30, width//2 + 30
    d.rectangle([(neck_left, top), (neck_right, top+neck_h)], outline="#0ea5e9", width=3, fill=(255,255,255,255))

    # Body (rectangle)
    body_top = top + neck_h
    d.rectangle([(left, body_top), (right, bottom)], outline="#0ea5e9", width=3, fill=(255,255,255,255))

    # Fill
    p = max(0, min(100, int(progress_pct)))
    fill_h = int((p / 100.0) * (bottom - body_top))
    fill_top = bottom - fill_h
    d.rectangle([(left+3, fill_top), (right-3, bottom-3)], fill=(59,130,246,200))
    # Little wave line
    d.line([(left+10, fill_top+10), (right-10, fill_top+10)], fill="#06b6d4", width=2)
    return img
...
with st.expander("💡 Daily hydration tip"):
    ...
    st.info(tip)
    b1, b2 = st.columns(2)
    if b1.button("Next tip"): st.session_state.tip_index += 1
    if b2.button("Previous tip"): st.session_state.tip_index = (st.session_state.tip_index - 1) % len(TIPS)
